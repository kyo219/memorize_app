...
- [クイズのモード一覧](#クイズのモード一覧)
  ...
  - [練習モード（書き取り → ランダムテスト）](#3-練習モード書き取り--ランダムテストpractice_offlinepy)
- [学習ノートを作る](#学習ノートを作る)
...
### 3. 練習モード（書き取り → ランダムテスト）`practice_offline.py`

```bash
uv run python scripts/practice_offline.py
```

**APIキー不要。** 数問（標準10問）をまとめて覚えてから、ランダム順でテストする「覚える→試す」の2段構えのモードです。

1. **データセット名を入力**（そのまま Enter で `default`）
2. **何問を練習するか**を入力（そのまま Enter で 10 問）
3. 出題範囲を選ぶ（`1`: 全問から / `2`: 間違い＋未回答から）
4. **📝 練習フェーズ** … 1問ずつ「日本語」と「英語（答え）」の両方を見ながら、英語を **5回書き取り**。`Enter` で次の問題へ
5. **🎯 テストフェーズ** … 覚えた問題を**ランダム順**で出題。日本語を見て英訳を入力し、自分で正誤を判定（`y` / `n`）

テストフェーズの成績はデータセットの CSV に記録されるので、間違えた問題はあとで復習モード（`2`）や学習ノートに回せます。
...
│   ├── practice_offline.py         # 練習：書き取り → ランダムテスト
